    ...
    <li className="flex items-center justify-between p-4 bg-white rounded-lg hover:bg-gray-50 transition-colors duration-200 gap-4 border">
      <div className="flex items-start gap-4 flex-1 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-gray-500 mt-1 truncate">{customer.phone}</p>
          <p className="text-sm text-gray-500 mt-1 truncate">{customer.address}</p>
        </div>
      </div>
...
const CustomerList: React.FC<CustomerListProps> = ({ customers, onDeleteCustomer }) => {
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border">
      <h3 className="text-xl font-bold text-gray-900 mb-4">Daftar Pelanggan</h3>
      {customers.length > 0 ? (
        ...
        <p className="text-gray-500 text-center py-8">Belum ada pelanggan.</p>
      )}
    </div>
  );
};
